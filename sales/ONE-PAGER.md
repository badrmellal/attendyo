...
| • **Tourne sur un simple serveur CPU.** Pas de GPU, pas de terminal propriétaire par porte. | • **Runs on a plain CPU server.** No GPU, no proprietary terminal per door. |
| • **Plus de cartes perdues.** Le visage est la clé — jamais perdue, prêtée ni clonée. | • **No more lost cards.** The face is the key — never lost, lent, or cloned. |
| • **Plusieurs portes, un seul serveur.** Ajoutez portes et caméras depuis la console web. | • **Many doors, one server.** Add doors and cameras from the web console. |
| • **Console web moderne** (FR / EN / AR), tableau de bord, présence quotidienne, export CSV. | • **Modern web console** (FR / EN / AR): dashboard, daily attendance, CSV export. |
